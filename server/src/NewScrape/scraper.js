...
class ApartmentListScraper {
    ...
    async init() {
        try {
            ...
            const pageHTML = await this.getPageHtml('https://www.apartmentlist.com/tx/san-antonio');
            ...
            await this.storeDataInFile('propertiesID.json', JSON.stringify(propertyIds));
    ...
    extractPropertiesId(jsonString) {
        try {
            ...
            return jsonData?.props?.pageProps?.component?.searchResult?.listingPins ?? null;
        } catch (error) {
    ...
    async processPropertyFetchingInBatch(apiToken, propertyIds) {
        try {
            ...
            const { listings } = await this.fetchProperyInformation(apiToken, propertyIds);
            fileData.push(...listings);
